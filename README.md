...
# Задача 2-4: Документация и финализация

## Описание
Финальная задача - создание полной документации проекта, инструкций по запуску и финализация Blog API.
...
|-------|-----|----------|-----------|
| GET | `/posts` | Получить все посты | - |
| GET | `/posts/{id}` | Получить пост по ID | `id` - ID поста |
| POST | `/posts` | Создать новый пост | JSON в теле запроса |
| PUT | `/posts/{id}` | Обновить пост | `id` - ID поста, JSON в теле |
| DELETE | `/posts/{id}` | Удалить пост | `id` - ID поста |

### Эндпоинты комментариев

| Метод | URL | Описание | Параметры |
|-------|-----|----------|-----------|
| GET | `/posts/{id}/comments` | Получить комментарии к посту | `id` - ID поста |
| POST | `/posts/{id}/comments` | Создать комментарий к посту | `id` - ID поста, JSON в теле |
| GET | `/comments/{id}` | Получить комментарий по ID | `id` - ID комментария |
| PUT | `/comments/{id}` | Обновить комментарий | `id` - ID комментария, JSON в теле |
| DELETE | `/comments/{id}` | Удалить комментарий | `id` - ID комментария |
...
| 404 | Ресурс не найден | Несуществующий ID |
| 405 | Метод не разрешен | Неподдерживаемый HTTP метод |
...
```bash
# Для базовой версии (1-1, 1-2, 1-3, 1-4)
pip install -r 1-1/requirements.txt

# Для версии с комментариями (2-1, 2-2)
pip install -r 2-1/requirements.txt

# Для версии с тестами (2-3)
...
cd 1-1  # Базовая версия
cd 1-2  # С моделями
cd 1-3  # С CRUD операциями
cd 1-4  # С обработкой ошибок
cd 2-1  # С комментариями
cd 2-2  # С валидацией
cd 2-3  # С тестами
...
curl -X POST http://localhost:5050/posts \
  ...
  -d '{"title": "Тестовый пост", "content": "Содержимое поста"}'
...
curl -X POST http://localhost:5050/posts/1/comments \
  -H "Content-Type: application/json" \
  -d '{"content": "Отличный пост!", "author": "Алексей"}'
```
...
├── 1-2/                    # Модели данных
├── 1-3/                    # CRUD операции для постов
├── 1-4/                    # Обработка ошибок и логирование
├── 2-1/                    # API эндпоинты для комментариев
├── 2-2/                    # Валидация данных
├── 2-3/                    # Тестирование
├── 2-4/                    # Документация и финализация
...
```
2024-01-01 12:00:00,123 - __main__ - INFO - Запрос: POST /posts от 127.0.0.1
2024-01-01 12:00:00,456 - __main__ - INFO - Создан новый пост с ID 1: Тестовый пост
2024-01-01 12:00:00,789 - __main__ - WARNING - Пост с ID 999 не найден
```
...
Blog API полностью реализован с:
- ✅ Полным набором CRUD операций
- ✅ Комплексной валидацией данных
- ✅ Защитой от атак
- ✅ Обработкой ошибок
